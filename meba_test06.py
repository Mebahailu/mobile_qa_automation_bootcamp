...
class Automation(webdriver):
    ...
    @classmethod
    def open_Echo_box(cls, keys):
        cls.find_element(AppiumBy.ACCESSIBILITY_ID, "Echo Box").click()

        cls.find_element(AppiumBy.ACCESSIBILITY_ID, "Echo Box").send_keys(f'{keys}')

        cls.find_element(AppiumBy.ACCESSIBILITY_ID, "//android.widget.TextView[@content-desc='save_button']").click() 
        
        return cls.find_element(AppiumBy.XPATH, f"//android.widget.TextView[@content-desc='{keys.lower()}']").get_text() 